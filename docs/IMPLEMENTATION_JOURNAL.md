...
## Android port milestone (DKC2Recomp-Android)

- Ported the SDL2 host to Android as a Gradle + NDK (r27) project: one
  `libdkc2.so` containing the generated AOT banks, the interpreter tier,
  the SNES runtime, recomp-ui, and a statically linked SDL 2.30.9.
- GLES2 adaptation of `desktop_present_sdl.c` (ES2 context, program-based
  quad, ES 1.00 reconstruct shaders, BGRA swizzle, core FBO captures);
  base program is mandatory, reconstruct remains optional with the same
  fallback policy.
- New `runner/android_main.c` entry (`SDL_main`) + `Dkc2RunGameHost` shim
  sharing the desktop gameplay loop; user data anchored to the app's
  internal storage via chdir.
- Touch controls implemented as an SDL2 virtual gamepad
  (`android_virtualpad.c` + `TouchControlsView.java`), reusing the
  standard-layout input path of physical controllers.
- ROM delivery via SAF picker with copy into app storage; runtime SHA-256
  verification unchanged and enforced.
- Local adaptation recorded: vendored recomp-ui (pinned revision) with an
  `__ANDROID__` guard around the posix_spawn-based Linux file dialogs;
  provenance documented in THIRD_PARTY_NOTICES.md.
- Verified locally: 12/12 host unit tests pass, ROM baseline verification
