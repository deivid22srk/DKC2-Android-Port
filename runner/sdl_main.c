...
/* Android entry shim: SDL_main lives in android_main.c and forwards here so
 * the desktop SDL host and the Android host share one gameplay loop. */
static int RunGame(const char *rom_path, RecompLauncherCSettings *settings);

int Dkc2RunGameHost(const char *rom_path, RecompLauncherCSettings *settings);
