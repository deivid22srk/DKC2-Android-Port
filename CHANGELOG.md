...
- First Android build (arm64-v8a, API 26+): Gradle/NDK project wrapping the
  shared SDL2 host, GLES2 video path, SDL virtual-gamepad touch controls,
  SAF ROM picker, and CI producing debug/release APKs from the private
  verified-ROM archive.
...
- Level-name cards are presented as bounded screens between black
  margins whatever their map size. Nothing authored exists beyond a
  card's 256 columns (the wider cards hold more painting on the right but
  only the map's wrap on the left), and a card on a 64-column map no
  longer takes the terrain path.
