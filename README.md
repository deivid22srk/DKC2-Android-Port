...
An Android port (arm64-v8a, Android 8.0+) builds from this same tree with
the same private-ROM workflow: see `docs/PORT_ANDROID.md`. CI produces
debug and release APKs on every push; the ROM is never committed and the
runtime verifies the same SHA-256 baseline as the desktop releases.
